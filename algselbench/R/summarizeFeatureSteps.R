...
summarizeFeatureSteps= function(astask) {
  ...
  # create prop table for the subsequent steo columns over their levels
  n = ncol(rs)
  res = dapply(rs[, 3:n], function(x)
  ...
  fc = astask$feature.costs[, 3:n]
  res2 = t(sapply(fc, function(x) c(
